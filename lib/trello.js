...
const getTrelloCards = function(trelloConfig, boardRegexp = '.*', listRegexp = '.*', cardRegexp = '.*') {
  ...
  return trello.getBoards('me')
    ...
    .then(boards => {
      ...
      const matchingBoards = boards.filter(board => boardNameRegexp.test(board.name))
      if(matchingBoards.length === 0) {
        return Promise.reject(`There is no board matching '${boardRegexp}' accessible with logged in user`)
      }
